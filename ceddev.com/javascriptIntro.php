...
<html>
	...
	<body>		
		<div id="container">			
			...
			<div id="flexDirRow">
				<a class="bouton_hor_nav" href="javascriptPoo.php">cliquez ici pour découvrir la Poo en JavaScript</a>
				<a class="bouton_hor_nav" href="index.php">cliquez ici pour voir des exemples</a>
			</div>
			<div id="central" style="margin-top: 25px;">
				<div id="allerEnHautIcone"><a href="#container"><img src="png/allerEnHaut.png"></a></div>
				<nav style="margin-left: -180px;margin-top: -80px;">
					<h2>					
						...
						<a href="#instructions"><h4>Les instructions if/else et else if</h4></a>
				...
				function RegEx($texte){
					...
					<img src="png/jschap1_2.png"/></ul>
					<!--AFFICHAGE VALEUR-->
					<h3 id="affVal">Affichage d'une valeur</h3>
					<ul><p>Voici comment afficher le résultat de valeurs dans la console de votre outils de développent sur firefox. Tapez ce code dans le fichier .js de votre dossier js</p>
					<div class="console">
					<?php
					...
					Adopter une convention de nommage : par exemple, les <em>nomsMontantTravauxMaison</em> ou <em>codeClientSuivant</em> respectent la norme <em>camelCase</em>.</p></ul>
					<!--INSTRUCTION IF-->
					...
					$texte ="var nombre = Number(prompt(\"Entrez un nombre  \"));
						if (nombre>0){<ul>console.log(nombre+ \" est positif\");</ul>}
						
						var nombre = Number(prompt(\"Entrez un nombre  \"));
						if (nombre >= 0){<ul>console.log(nombre + \" est positif ou nul\");</ul>}";
						RegEx($texte);
					?>
					</div>
					<p>La console affichera : </p>
					<img src="png/jschap1_6.png"/></ul>
					<!--INSTRUCTION ELSE-->
					...
					<?php
					$texte ="var nombre = Number(prompt(\"Entrez un nombre\"));
						if (nombre>0){<ul>console.log(nombre + \" est positif\");</ul>}else{<ul>console.log(nombre+ \" est négatif ou nul\");</ul>}
						
						var nombre = Number(prompt(\"Entrez un nombre\"));
						if (nombre>0){<ul>console.log(nombre + \" est positif\");</ul>}else{<ul>if (nombre <0){<ul>console.log(nombre + \" est négatif\");</ul>}else{<ul>console.log(nombre+ \" est nul\");</ul>}</ul>}";
						RegEx($texte);
					?>
					</div>
					<p>Quand on entre 0 et 5 et la console affiche : </p>
					<img src="png/jschap1_7.png"/></ul>
					<!--INSTRUCTION ELSE IF-->
					...
					<?php
					$texte ="var nombre = Number(prompt(\"Entrez un nombre\"));
						if (nombre > 0) {<ul>console.log(nombre + \" est positif\");</ul>} else if (nombre < 0) {<ul>console.log(nombre + \" est négatif\");</ul>} else {<ul>   console.log(nombre + \" est nul\");</ul>}";
						RegEx($texte);
					?>
					</div>
					...
					<?php
					$texte ="var nombre = Number(prompt(\"Entrez un nombre\"));
						if ((nombre >= 0) && (nombre <= 100)){<ul>console.log(nombre + \" est compris entre 0 et 100\");</ul>}

						var nombre = Number(prompt(\"Entrez un nombre\"));
						if ((nombre < 0) || (nombre > 100)){<ul>console.log(nombre + \" est dans l'intervale 0, 100 \");</ul>}
						
						var nombre = Number(prompt(\"Entrez un nombre\"));
						if (!(nombre > 100)){<ul>console.log(nombre + \" est inférieur ou égal à 100\");</ul>}";
						RegEx($texte);
					?>
					</div>
					<p>Avec 5 , 110 et -10 la console affichera : </p>
					<img src="png/jschap1_9.png"/></ul>
					<!--INSTRUCTION SWITCH-->
					...
					$texte ="var meteo = prompt(\"Quel temps fait-il dehors ?\");
						if (meteo === \"soleil\") {<ul>console.log(\"Sortez en t-shirt.\");</ul>} else if (meteo === \"vent\") {<ul>console.log(\"Sortez en pull.\");</ul>} else if (meteo === \"pluie\") {<ul>console.log(\"Sortez en blouson.\");</ul>} else if (meteo === \"neige\") {<ul>console.log(\"Restez au chaud à la maison.\");</ul>} else {<ul>console.log(\"Je n'ai pas compris !\");</ul>}";
						RegEx($texte);
					?>
					</div>
					...
					<?php
					$texte ="var meteo = prompt(\"Quel temps fait-il dehors ?\");
						switch (meteo) {<ul>case \"soleil\":<ul>console.log(\"Sortez en t-shirt.\");
						    break;</ul>
						case \"vent\":<ul>console.log(\"Sortez en pull.\");
						    break;</ul>
						case \"pluie\":<ul>console.log(\"Sortez en blouson.\");
						    break;</ul>
						case \"neige\":<ul>console.log(\"Restez au chaud à la maison.\");
						    break;</ul>
						default:<ul>console.log(\"Je n'ai pas compris !\");</ul>}";
						RegEx($texte);
					?>
					</div>
					<p>Les deux programme donne le même résultat :</p>
					<img src="png/jschap1_10.png"/></ul>
					<!--LES BOUCLE-->
					...
					$texte ="console.log(\"Début du programme\");
						var nombre = 1;
						while (nombre <= 5) {<ul>console.log(nombre);
						    nombre++;</ul>}
					...
					<?php
					$texte ="console.log(\"Début du programme\");
						for (var compteur = 1;compteur <= 5;compteur++){<ul>console.log(compteur);</ul>}
						console.log(\"Fin du programme\");";
